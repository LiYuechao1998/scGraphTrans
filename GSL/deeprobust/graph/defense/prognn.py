...
class ProGNN:
    ...
    def fit(self, features, adj, labels2, idx_train, idx_val, **kwargs):
        """Train Pro-GNN.
        ...
        for epoch in range(args.epochs):
            if args.only_gcn:
                self.train_gcn(epoch, features, estimator.estimated_adj
                        ,labels2, idx_train, idx_val)
            else:
                for i in range(int(args.outer_steps)):
                    self.train_adj(epoch, features, adj,labels2,
                            idx_train, idx_val)
    ...
    def train_gcn(self, epoch, features, adj, labels2, idx_train, idx_val):
        args = self.args
        ...
        loss_train = F.mse_loss(output[idx_train], labels2[idx_train])
        ...
        loss_val = F.mse_loss(output[idx_val], labels2[idx_val])
        acc_val = loss_val
        ...
        if args.debug:
            if epoch % 1 == 0:
                print('Epoch: {:04d}'.format(epoch+1),
                      ...
                      'mse_train: {:.4f}'.format(acc_train.item()),
                      #'acc_train: {:.4f}'.format(acc_train.item()),
                      'loss_val: {:.4f}'.format(loss_val.item()),
                      #'acc_val: {:.4f}'.format(acc_val.item()),
                      'mse_val: {:.4f}'.format(acc_val.item()),
                      'time: {:.4f}s'.format(time.time() - t))



    def train_adj(self, epoch, features, adj, labels2, idx_train, idx_val):
        estimator = self.estimator
        args = self.args
        if args.debug:
            print("\n=== train_adj ===")
        ...
        loss_fro = torch.norm(estimator.estimated_adj - adj, p='fro')#A-S
        ...
        loss_gcn = F.mse_loss(output[idx_train], labels2[idx_train])
        ...
        print('output[idx_train][:10]')
        print(output[idx_train][:10])  # 打印前 10 个 output 元素
        ...
        if acc_val > self.best_val_acc:
            ...
            if args.debug:
                ...
                print(f'\t=== saving current graph/gcn, best_val_mse: %s' % self.best_val_acc.item())
    ...
    def test(self, features,labels2, idx_test):
        ...
        loss_test = F.mse_loss(output[idx_test], labels2[idx_test])
        acc_test = loss_test
        #acc_test = accuracy(output[idx_test], labels[idx_test])
        print("\tTest set results:",
              "loss= {:.4f}".format(loss_test.item()),
              "mse= {:.4f}".format(acc_test.item()))
              #"accuracy= {:.4f}".format(acc_test.item()))
        return adj, acc_test.item()
