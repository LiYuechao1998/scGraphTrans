...
def preprocess(adj, features, labels2, preprocess_adj=False, preprocess_feature=False, sparse=False, device='cpu'):
    ...
    labels2 = torch.FloatTensor(labels2.todense())
    if sparse:
        adj = sparse_mx_to_torch_sparse_tensor(adj)
        features = sparse_mx_to_torch_sparse_tensor(features)
    else:
        if sp.issparse(features):
            print('sp.issparse(features)')
            features = torch.FloatTensor(np.array(features.todense()))
        else:
            print('no sp.issparse(features)')
            features = torch.FloatTensor(features)
        adj = torch.FloatTensor(adj.todense())

    return adj.to(device), features.to(device), labels2.to(device)
